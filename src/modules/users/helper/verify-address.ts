...
export class VerifyAddress {
  ...
  public static async checkRecoverSameAddress(address: string, signature: string, message: string): Promise<boolean> {
    if (!VerifyAddress.instance) {
      VerifyAddress.instance = new VerifyAddress();
    }
    const recover = await VerifyAddress.instance.web3.eth.accounts.recover(message, signature);
